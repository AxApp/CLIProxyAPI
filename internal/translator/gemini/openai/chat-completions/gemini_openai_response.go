...
func ConvertGeminiResponseToOpenAI(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if candidates.IsArray() {
		candidates.ForEach(func(_, candidate gjson.Result) bool {
			...
			if stopReasonResult := gjson.GetBytes(rawJSON, "stop_reason"); stopReasonResult.Exists() && stopReasonResult.String() != "" {
				p.UpstreamFinishReason[candidateIndex] = strings.ToUpper(stopReasonResult.String())
			} else if finishReasonResult := candidate.Get("finishReason"); finishReasonResult.Exists() {
			...
			if upstreamFinishReason != "" && usageExists {
				finishReason := "stop"
				if p.SawToolCall[candidateIndex] {
