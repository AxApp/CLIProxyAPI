...
func collectRequestToolNames(data []byte) map[string]string {
	...
	for _, tool := range parsed.Get("tools").Array() {
		...
		record(tool.Get("function.name").String())
	}

	toolChoice := parsed.Get("tool_choice")
	if toolChoice.Type == gjson.String {
		switch choice := strings.TrimSpace(toolChoice.String()); strings.ToLower(choice) {
		case "", "auto", "any", "none", "required":
		default:
			record(choice)
		}
	} else if toolChoice.Exists() {
		record(toolChoice.Get("name").String())
		record(toolChoice.Get("function.name").String())
	}
