...
type Result struct {
	...
	// Latency records the upstream execution duration when available.
	...
	for idx, execModel := range execModels {
		...
		if bootstrapErr != nil {
			...
			if isRequestInvalidError(bootstrapErr) {
				...
				result := Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: false, Error: rerr, Latency: latency}
		...
		if closed && len(buffered) == 0 {
			...
			result := Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: false, Error: emptyErr, Latency: latency}
			m.MarkResult(ctx, result)
			if idx < len(execModels)-1 {
				lastErr = emptyErr
				continue
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		streamReq := stripGenerateForWebsocketHTTPFallback(execCtx, auth, req)
		streamResult, errStream := m.executeStreamWithModelPool(execCtx, executor, auth, provider, streamReq, opts, routeModel, models, pooled)
		if errStream != nil {
			if errCtx := execCtx.Err(); errCtx != nil {
				admission.Lease.Release(execCtx)
				return nil, errCtx
...
func stripGenerateForWebsocketHTTPFallback(ctx context.Context, auth *Auth, req cliproxyexecutor.Request) cliproxyexecutor.Request {
	if !cliproxyexecutor.DownstreamWebsocket(ctx) || authWebsocketsEnabled(auth) || len(req.Payload) == 0 {
		return req
	}
	payload, errDelete := sjson.DeleteBytes(req.Payload, "generate")
	if errDelete != nil {
		return req
	}
	req.Payload = payload
...
func (m *Manager) tryAntigravityCreditsExecute(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, bool) {
	...
	for _, c := range candidates {
		...
		for _, upstreamModel := range models {
			...
			result := Result{AuthID: c.auth.ID, Provider: c.provider, Model: resultModel, Success: errExec == nil, Latency: time.Since(startedAt)}
			if errExec != nil {
				result.Error = &Error{Message: errExec.Error()}
				if se, ok := errors.AsType[cliproxyexecutor.StatusError](errExec); ok && se != nil {
					result.Error.HTTPStatus = se.StatusCode()
