...
func TestResponsesWebsocketReleasesPinnedAuthAfterRouteGuardBlock(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	registry.GetGlobalRegistry().RegisterClient(authA.ID, authA.Provider, []*registry.ModelInfo{{ID: "guard-model"}})
	registry.GetGlobalRegistry().RegisterClient(authB.ID, authB.Provider, []*registry.ModelInfo{{ID: "guard-model"}})
	t.Cleanup(func() {
		registry.GetGlobalRegistry().UnregisterClient(authA.ID)
		registry.GetGlobalRegistry().UnregisterClient(authB.ID)
	})
	...
	if errWrite := conn.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","model":"guard-model","input":[{"type":"message","id":"msg-1"}]}`)); errWrite != nil {
	...
	if errWrite := conn.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","previous_response_id":"resp-auth-a-1","input":[{"type":"message","id":"msg-2"}]}`)); errWrite != nil {
	...
	authBPayload := authBPayloads[0]
	if gjson.GetBytes(authBPayload, "previous_response_id").Exists() {
		t.Fatalf("previous_response_id leaked after guard failover: %s", authBPayload)
	}
	authBInput := gjson.GetBytes(authBPayload, "input").Raw
	if !strings.Contains(authBInput, `"id":"msg-1"`) || !strings.Contains(authBInput, `"id":"msg-2"`) {
		t.Fatalf("auth-b replay input missing expected transcript items: %s", authBInput)
